<template>
  <div class="vdp-datepicker date-input" :class="wrapperClass">
    <span :class="{'input-group' : bootstrapStyling}">
      <span class="datepicker__calendar-button" :class="{'input-group-addon' : bootstrapStyling}" v-if="calendarButton" @click="showCalendar()"><i :class="calendarButtonIcon"><span v-if="calendarButtonIcon.length === 0">&hellip;</span></i></span>
      <input
      ...
      <span class="datepicker__calendar-icon" :class="{'input-group-addon' : bootstrapStyling}" v-if="calendarSvgButton" @click="showCalendar()"><svg :class="calendarSvgIcon" class="icon date-input__icon"><use xlink:href="svg/svg.svg#icon-calendar"></use></svg></span>
    </span>
        <!-- Day View -->
