...
export default {
  ...
  computed: {
    ...
    ...mapGetters({
      confirmDelete: 'citiesStreetsSearch/delete',
      deleteCitiesStreets: 'citiesStreetsSearch/deleteCitiesStreets',
      streetsList: 'citiesStreetsSearch/streetsList'
    })
  },
  watch: {
    ...
    'confirmDelete' () {
      if ((this.name === 'citiesList' || this.name === 'streetsList')
        && (this.deleteCitiesStreets.cityToDelete !== undefined || this.deleteCitiesStreets.streetsToDelete !== undefined)) {
        let self = this;
        if (this.name === 'citiesList') {
          console.log('citiesList');
          self.removeElement(this.deleteCitiesStreets.cityToDelete);
          this.deleteCitiesStreets.cityToDelete = undefined;
        } else {
          console.log('streetsList');
          this.deleteCitiesStreets.streetsToDelete.forEach(function (item) {
            self.removeElement(item);
          });
          this.deleteCitiesStreets.streetsToDelete = undefined;
        }
        if (this.deleteCitiesStreets.cityToDelete === undefined && this.deleteCitiesStreets.streetsToDelete === undefined) {
          this.$store.state.citiesStreetsSearch.delete = false;
        }
        // this.removeElement(this.temp.cityToDelete);
      }
    }
  },
  methods: {
    ...
    removeElement (option) {
      ...
      if (this.name === 'citiesList' && this.streetsList.show && this.confirmDelete === false) {
        let streetsToDelete = this.streetsList.show.filter(function (item) {
          return option.id === item.city;
        });
        if (streetsToDelete.length !== 0) {
          this.deleteCitiesStreets.streetsToDelete = streetsToDelete;
          this.deleteCitiesStreets.cityToDelete = option;
          this.$parent.$refs.modalConfirm.openModal();
          return;
        }
      }
      ...
      /* if (this.deleteCitiesStreets.cityToDelete !== undefined && this.deleteCitiesStreets.streetsToDelete !== undefined) {
       ...
       console.log(this.$store.state.citiesStreetsSearch.delete);*/
      /* istanbul ignore else */
      if (this.closeOnSelect) this.deactivate()
    },
    /**
